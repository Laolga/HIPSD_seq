...
def find_match(clust_df, d_tresh, saturated=False):
    ...
    for idx1 in range(dist.shape[0]):
        ...
        d = dist[idx1, idx2]
        if d < d_tresh and clust_df.iloc[idx1].pass_min == False and clust_df.iloc[idx2].pass_min == False:
            # merge 2 cells with unsufficient coverage
            dist_tup.append(
                (to_merge_df.index[idx1], to_merge_df.index[idx2], d))
        elif d < d_tresh and clust_df.iloc[idx1].pass_min == False and clust_df.iloc[idx2].pass_max:
...
def merge_cells(d_tresh, clust_df, key, adata, min_t=200, max_t=1100):
    ...
    while not_passed != 0:
        ...
        for tup in filtered_tup:
            ...
            new_pcs = np.array([getattr(tup1, x) for x in pcs]) + \
                np.array([getattr(tup2, x) for x in pcs])*0.5
            new_t_min = new_cov > min_t
            new_t_max = new_cov <= max_t
            new_df.append((new_id, new_cov, new_n_cells, new_x,
                          new_y, *new_pcs, new_t_min, new_t_max))
            merged.add(tup1.cl)
            merged.add(tup2.cl)
...
# sample_path = "/home/o313a/olga/aurelie_data/pdx_paired/"
# adata_path = sample_path + "adata_1mb.h5ad"
# adata = sc.read_h5ad(adata_path)
# clust_df = pd.read_csv(sample_path + "pre_metacells.csv", index_col=0)
# num_comp = adata.obsm["X_pca"].shape[1]
# max_covergae = 1100
# min_coverage = 200

# dist = pairwise_distances(clust_df[pcs])
# ts = np.quantile(dist[np.triu_indices(dist.shape[0], k=1)],
#                  np.linspace(0.01, 1, num=10))
# new_df, cell_dict, = merge_cells(
#     ts[6], clust_df, "leiden_small", adata, min_coverage, max_covergae)
